...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        print("CORE DATA PATH: \(NSSearchPathForDirectoriesInDomains(.applicationSupportDirectory, .userDomainMask, true).last! as String)")
        
        //if user is already logged in, directly go to Arrivals
        if  UserDefaults.standard.bool(forKey: isUserLoggedIn) {
            ...
            let loggedUserId = UserDefaults.standard.value(forKey: loggedInUserId) as! String
            ...
            do {
                ...
                for user in users {
                    if user.id == loggedUserId {
                        let allUserClothsPairs = user.clothsPair?.allObjects as? [Images]
                        if (allUserClothsPairs?.count)! > 0 {
                            shoppingVC.pantShirtImages = allUserClothsPairs!
                        }
                        self.window?.rootViewController = shoppingNavigationController
                        self.window?.makeKeyAndVisible()
                        return true
                    }
                }
            } catch  {
                print("Error while fetching logged in user")
            }
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        ...
        */
        let container = NSPersistentContainer(name: "Krixi_Demo")
